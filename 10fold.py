# -*- coding: utf-8 -*-
"""
Created on Sat Sep 14 13:40:01 2019
...
# -*- coding: utf-8 -*-
"""
Created on Sat Sep 14 12:35:10 2019
...
#for each categorical variable, if it has distinct values more than 5, we use the ChiMerge to merge it
path = './'
admissions = pd.read_csv(path+'38mRNA_38gene.csv',header =0, index_col=0, encoding='gbk')
admissions =admissions.stack().unstack(0)
admissions["actual_label"] = admissions["target"]
admissions=admissions.drop("target", axis=1)


shuffled_index = np.random.permutation(admissions.index)
shuffled_admissions = admissions.loc[shuffled_index]
admissions = shuffled_admissions.reset_index(drop=True)
admissions.ix[0:51, "fold"] = 1
admissions.ix[52:103, "fold"] = 2
admissions.ix[104:155, "fold"] = 3
admissions.ix[156:207, "fold"] = 4
admissions.ix[208:259, "fold"] = 5
admissions.ix[260:311, "fold"] = 6
admissions.ix[312:363, "fold"] = 7
admissions.ix[364:415, "fold"] = 8
admissions.ix[416:468, "fold"] = 9
admissions.ix[469:522, "fold"] = 10
...
def train_and_test(df, folds):
    ...
    for fold in folds:
        ...
        train_iteration_one = admissions[admissions["fold"] != fold]
        test_iteration_one = admissions[admissions["fold"] == fold]
        
        trainData=train_iteration_one.drop(columns=['actual_label','fold'])
        y_train=train_iteration_one['actual_label']
               

        X_test=test_iteration_one.drop(columns=['actual_label','fold'])
        y_test=test_iteration_one['actual_label']

        
        trainData['actual_label']=y_train
        allFeatures = list(trainData.columns)
        allFeatures.remove('actual_label')
        ...
        for col in categorical_var:
            ...
            if len(set(trainData[col]))>5:
                ...
                encoding_result = BadRateEncoding(trainData, col, 'actual_label')
                ...
                if min(bad_bin) == 0:
                    ...
                    WOE_IV = CalcWOE(trainData, col1, 'actual_label')
                    var_WOE[col1] = WOE_IV['WOE']
                    var_IV[col1] = WOE_IV['IV']
                    #del trainData[col]
                    deleted_features.append(col)
                else:
                    WOE_IV = CalcWOE(trainData, col, 'actual_label')
        ...
        for col in numerical_var:
            ...
            if -1 in set(trainData[col]):
                ...
                special_attribute = []
            cutOffPoints = ChiMerge_MaxInterval(trainData, col, 'actual_label',special_attribute=special_attribute)
            ...
            BRM = BadRateMonotone(trainData, col1, 'actual_label',special_attribute=special_attribute)
            if not BRM:
                for bins in range(4,1,-1):
                    cutOffPoints = ChiMerge_MaxInterval(trainData, col, 'actual_label',max_interval = bins,special_attribute=special_attribute)
                    trainData[col1] = trainData[col].map(lambda x: AssignBin(x, cutOffPoints,special_attribute=special_attribute))
                    BRM = BadRateMonotone(trainData, col1, 'actual_label',special_attribute=special_attribute)
                    if BRM:
                        break
        ...
        acc.append(accuracy_score(y_test, y_pred_int))
        ...
        print("average_AUC1:",average_auc)
        print("average_ACC1:",average_acc)
        
        features_selection_dic[fold]=features_selection
        print("feature selection:",features_selection_dic)       

train_and_test(admissions, fold_ids)
